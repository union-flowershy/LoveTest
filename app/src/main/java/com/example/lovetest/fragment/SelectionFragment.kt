...
class SelectionFragment : Fragment(), View.OnClickListener {
    ...
    override fun onClick(v: View?) {
        ...
        when(v?.id) {       //물음표를 써야하는 이유는 v가 null이 아니면 아이디를 가져오고 null이라면 전체는 null을 반환
            ...
            R.id.btn_back -> {
                navController.popBackStack()    //스택에 있는걸 가져옴 (뒤로가기) 스택은 계속 쌓임
            }
        }
    }

    fun navigateWithIndex(index: Int) {
        val bundle = bundleOf("index" to index) //키는 인덱스
